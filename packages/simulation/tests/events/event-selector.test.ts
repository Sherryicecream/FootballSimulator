...
describe('filterEligibleEvents', () => {
  it('根据年龄过滤事件', () => {
    const result = filterEligibleEvents(mockEvents, {
      ...
      storyState: { activeStorylines: [], completedStoryIds: [], cooldowns: {}, pendingDelayedEffects: [] },
    });
  ...
  it('排除冷却期中的事件', () => {
    const result = filterEligibleEvents(mockEvents, {
      age: 16, reputation: 20, season: 2024, week: 1,
      storyState: {
        activeStorylines: [], completedStoryIds: [], cooldowns: { youth_training: 4 }, pendingDelayedEffects: [],
      },
    });
  ...
  it('排除已互斥的事件', () => {
    const result = filterEligibleEvents(mockEvents, {
      age: 16, reputation: 20, season: 2024, week: 1,
      storyState: {
        activeStorylines: [], completedStoryIds: ['youth_training'],
        cooldowns: {}, pendingDelayedEffects: [],
      },
    });
    ...
    const eventsWithExclude = [
      ...mockEvents,
      {
        id: 'post_training', version: 1, category: 'china-youth', rarity: 'common',
        title: '训练后', description: '训练后的总结',
  ...
  it('需要前置故事线的事件', () => {
    const eventsWithRequire = [
      ...mockEvents,
      {
        id: 'advanced_training', version: 1, category: 'china-youth', rarity: 'uncommon',
        title: '进阶训练', description: '需要先完成基础训练',
