...
- 增加当前故事进度、最近选择和待触发原因的可解释展示；月报只展示已发生内容和下一步关注。
- 保持同月互动事件上限、冷却、事实条件和旧存档恢复。

### 模块 3：比赛日节奏与可玩性增强（待执行）

- 在月度流程中增加关键比赛前后的小节点、对手强弱和表现反馈；比赛结果仍由 simulation 决定。
- 以比赛密度、决策密度、伤病率和赛季完成率做批量校准。

### 模块 4：安全的本地 AI 叙事层（待执行）
