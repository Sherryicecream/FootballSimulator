...
- [x] 先添加 simulation 与 web 的失败回归测试，覆盖证据 ID、目标进度和页面展示。
- [x] 从现有账本派生故事选择、关键比赛、关系转折、国家队、合同、健康与阶段节点；过滤普通比赛，兼容旧账本上下文缺失。
- [x] 从合同、职业出场、国家队、赛季历史和故事事实派生长期目标；不改变存档结构、阶段机或随机序列。
- [x] 在退役回顾页展示足球场景横幅、目标进度卡、语义图标和时间节点回放，同时保留逐季时间线。
- [x] 运行专项测试、全量门禁、E2E 和 1,000 赛季平衡命令。

实际结果：新增 2 个专项测试文件、2 个测试通过；回放证据可由账本/赛季历史对账，存档重新加载后由同一派生函数得到同一结果。页面新增长期目标与全生涯回放，场景横幅在退役页作为装饰地标处理，兼容原有单区域恢复测试。
