...
- 承诺 `kept` 率约 70%–95%（毕业资格门槛筛除弱者后兑现为主流）；`broken` 以伤病归因为主，俱乐部原因在批量中罕见（0–10%），归因逻辑由单元测试锁定。
- 角色晋升合理：3 季内达到"首发"身份的比例约 15%–40%。
- 每次一线队出场的平均分钟中位数约 55–85（首发为主的球员接近打满）；预备队出场随角色下降而上升。
- 严重伤病率高于青训期但可控（<4%/季；批量口径为 3 季内至少一次重伤的球员占比 <12%）。
