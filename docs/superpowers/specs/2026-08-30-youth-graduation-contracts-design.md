...
| 当前能力    | 16 项可见属性按位置权重加权后 ≥ 俱乐部层级门槛（青训机构 `competitionLevel` + 8，即本机构比赛级别对应的签约线）                         |
...
`interest ≥ 0.45` 的俱乐部进入要约池；池中按 `interest` 排序取 2–4 份（种子决定数量）；不足 2 份时插入 1 份低层级保底要约（层级 ≤4）。候选俱乐部来自 `packages/content` 的虚构俱乐部表（≥12 家，层级 3–8，含位置需求与青训周期标注），启动前经内容校验。
